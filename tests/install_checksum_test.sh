...
test_valid_checksum_with_star_and_path_prefix() {
  archive="$test_tmp/star-path.tar.gz"
  checksums="$test_tmp/star-path-checksums.txt"
  write_archive_bytes "$archive"
  digest="$(real_sha256 "$archive")"
...
test_missing_archive_file_fails() {
  archive="$test_tmp/does-not-exist.tar.gz"
  checksums="$test_tmp/missing-file-checksums.txt"
...
run_test "valid checksum with star and path prefix" test_valid_checksum_with_star_and_path_prefix
...
run_test "missing archive file fails" test_missing_archive_file_fails
